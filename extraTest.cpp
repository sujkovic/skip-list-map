...
template <typename T>
void insertTimeComplexity()
{
    using namespace std::chrono;
    for (int i = 1000; i <= 10000000; i *= 10)
    {
        TimePoint start = system_clock::now();
        ascendingInsert<T>(i, false);
        TimePoint end = system_clock::now();
        Milli elapsed = end - start;
        double timeTaken = elapsed.count() / i;
        std::cout << "For " << i << " elements, each insert took on average " << timeTaken << " milliseconds"
                  << ", log(n) = " << log(i) << ", log(insert time) = " << log(timeTaken) << std::endl;
    }
    std::cout << std::endl
              << "The above shows if you take the log of the map size and the insert time at that map size, both increase linearly together as map size increases" << std::endl;
}
...
int main()
{
    ...
    {
        dispTestName("Adrian Insert Time Complexity", m);
        insertTimeComplexity<cs440::Map<int, int>>();
    }
